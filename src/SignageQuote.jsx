...
// ═══════════════════════════════════════════════════════════════
//   EMAIL CONFIG — set up at https://www.emailjs.com (free 200/mo)
//   1. Sign up, add an email service (Gmail/Outlook/etc).
//   2. Create a template with these vars:
//        {{customer_name}} {{customer_email}} {{customer_phone}}
//        {{customer_message}} {{quote_total}} {{quote_breakdown}}
//      Set the template "To Email" to mick@strikeprint.com.au.
//      Add TWO Attachments to the EmailJS template:
//        - Filename "mockup.jpg"   Content: {{mockup_attachment}}
//        - Filename "original.jpg" Content: {{original_attachment}}
//   3. Paste the IDs below.
// ═══════════════════════════════════════════════════════════════
const EMAILJS_PUBLIC_KEY  = "";        // e.g. "Abc123xyzPublicKey"
const EMAILJS_SERVICE_ID  = "";        // e.g. "service_a1b2c3d"
const EMAILJS_TEMPLATE_ID = "";        // e.g. "template_x9y8z7w"
const RECIPIENT_EMAIL     = "mick@strikeprint.com.au";
...
export default function SignageQuoteBuilder() {
  ...
  const onGenerate = async () => {
    ...
    if (placements.length === 0) { setError('Drag at least one sign template onto your photo'); return; }
    setGenerating(true);
    setError(null);
    try {
        ...
        {!photoDataUrl && (
          <div className="max-w-3xl mx-auto anim-fadeup">
            ...
            <p className="text-sm mt-4 mb-6 leading-relaxed" style={{ color: BRAND.textMuted }}>
              Upload a clear photo of the storefront, building, vehicle, or interior where the signage will go. You'll drag templates onto it next.
            </p>
    ...
    <div className="space-y-8">
      <div className="anim-fadeup">
        ...
        <p className="text-sm mt-4 leading-relaxed" style={{ color: BRAND.textMuted }}>
          Click a template from the panel below to add it. Drag to position, drag the corner to resize. Click any placed sign to edit its text or swap colours.
        </p>
      ...
      <div className="flex gap-2 lg:gap-4 items-start">
        {/* Sidebar: Templates panel — left side, narrow on mobile, wider on desktop */}
        <aside className="anim-slideL flex-shrink-0 w-[88px] lg:w-[260px]">
          <div className="sticky top-2 lg:top-4">
            {/* Sidebar header */}
            <div className="flex items-center justify-between gap-1 mb-2 lg:mb-3 px-2 lg:px-3 py-2"
              style={{
              ...
              <div className="flex items-center gap-1.5 min-w-0">
                ...
                <span className="text-[9px] lg:text-[10px] uppercase tracking-[0.2em] lg:tracking-[0.25em] truncate"
                  style={{ fontFamily: "'JetBrains Mono', monospace", color: BRAND.boltAmber }}>
                  <span className="hidden lg:inline">Templates</span>
                  <span className="lg:hidden">Signs</span>
                </span>
              </div>
              <span className="text-[9px] lg:text-[10px] flex-shrink-0"
                style={{ fontFamily: "'JetBrains Mono', monospace", color: BRAND.textDim }}>
                {TEMPLATE_LIBRARY.length}
              </span>
            </div>

            {/* Vertical scroll container — sticky-pinned to viewport */}
            <div className="max-h-[calc(100vh-9rem)] lg:max-h-[calc(100vh-12rem)] overflow-y-auto pr-1 flex flex-col gap-2"
              style={{
                scrollbarWidth: 'thin',
            ...
            {/* Sidebar footer hint — desktop only, no room on mobile */}
        ...
        <div className="space-y-4 flex-1 min-w-0">
          ...
          {selected ? (
            ...
            <div className="p-4 text-center text-xs uppercase tracking-widest"
              ...
              {placements.length === 0
                ? 'Click a template to add it to the photo'
...
function ContactForm({ result, originalPhoto, fmt }) {
  ...
  // Lazy-load the EmailJS SDK from CDN once, when needed
  const loadEmailJS = () => new Promise((resolve, reject) => {
    ...
    s.onload = () => {
      ...
      } else reject(new Error('EmailJS failed to load'));
    };
    s.onerror = () => reject(new Error('Could not load EmailJS — check internet connection'));
  ...
  const onSubmit = async (e) => {
    ...
    // Always save the mockup + original locally first — works even if email is misconfigured
    ...
    // If EmailJS isn't configured, fall back to a mailto link
    ...
    try {
      ...
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
        ...
        mockup_attachment: result.composite || '',
        original_attachment: originalPhoto || ''
      });
      setStatus('sent');
      ...
      console.error(err);
      setStatus('error');
      setStatusMsg('Email send failed: ' + (err.text || err.message || 'unknown error') + '. Mockup was still saved to your device.');
    }
  };
